...
export function renderFaqAccordion(dom) {
  ...
  FAQ_PUBLIC_COPY.forEach(function (item) {
    ...
    html += '<summary class="flex justify-between items-center font-medium cursor-pointer list-none p-5 text-sm text-[#2D2D2D]">';
    ...
    html += '</summary>';
    html += '<div class="text-[#666666] text-sm mt-3 font-light px-5 pb-5 leading-relaxed">' + item.a + '</div>';
    html += '</details>';
  });
...
export function renderEmbedDossierWizard(dom, userData, deps) {
  ...
  if (grid) {
    ...
    if (symptoms.length === 0 || symptoms.includes('saudavel')) {
      grid.innerHTML = '<div class="bg-[#F0FDF4] p-4 border border-[#10B981]/20 rounded-xl text-sm text-[#065F46] font-light">Integridade preservada — foco em prevenção e manutenção.</div>';
    } else {
      symptoms.slice(0, 3).forEach(function (symp) {
        if (SYMPTOM_CARDS[symp]) grid.innerHTML += SYMPTOM_CARDS[symp];
      });
      if (symptoms.length > 3) {
        grid.innerHTML += '<p class="text-xs text-[#999999] text-center">+' + (symptoms.length - 3) + ' indicador(es) na avaliação presencial.</p>';
      }
    }
  }
